...
module.exports = {
     ...
     */
    oracleExecuteByCallback: async function(oracledb, sql, params, dbCallback) {
        //回调函数参数封装对象
        var resultData = {
            status: 0 //默认为失败状态
        };
     ...
     */
    oracleExecuteByPromise: function(db, sql, params) {
     ...
     */
    mysqlExecuteByCallback: function(db, sql, params, dbCallback) {
        //返回结果
        var resultData = {
            status: 0 //默认为失败状态
        };
     ...
     */
    mysqlExecuteByPromise: function(db, sql, params) {
        return new Promise(async(resolve, reject) => {
            try {
                db.getConnection(function(err, connection) {
                    if (err) {
